...
3.2 **交互中断规则（强制）**：版本开发过程中，仅在以下两类场景允许中断并向用户确认：
   - 缺少关键输入导致无法继续（如凭据、账户ID、目标环境等）。
   - 高风险不可逆操作（如 push/tag 发布、生产级安全/计费策略切换）。
   其余情况必须持续开发直到该版本开发包完成，再一次性汇总交付。
4. **结果输出顺序**：
   - 问题根因
   - 修复方案
   - 改动清单（文件路径）
...
6.1 **版本发布节奏（强制）**：
   - 日常开发阶段：仅给“建议版本号”，不执行发布 commit/tag。
   - 仅在“项目阶段完成/用户明确要求发布”时，统一执行一次版本发布。
   - 禁止“一个步骤一个版本”。
...
> 目标：解决“AGENTS/CHANGELOG/实际代码版本不一致”问题，并避免碎片化频繁发版。

发布前提（必须满足其一）：
1. 用户明确下达“现在发布版本”指令；
2. 当前项目里程碑/阶段性目标全部完成（一次性收口）。

未满足发布前提时：
- 只做本地功能提交（feature/fix/test/docs）。
- 输出“建议版本号”，不更新 `CHANGELOG.md` 版本段，不打 tag。
